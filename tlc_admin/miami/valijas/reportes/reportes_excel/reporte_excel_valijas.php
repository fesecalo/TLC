...
//PARAMENTROS	
	...
	//CABECERA
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A1', 'Valijas procesadas')
	...
	//TITULOS
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A7', 'Valijas procesadas')
				->setCellValue('A8', 'Cincho')
	...
	//FORMATO DE LOS TITULOS
			...
			foreach ($sql_valija_cerrada as $valijas){
				$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('A'.$y, $valijas->cincho)
							->setCellValue('B'.$y, $valijas->peso_kilos/0.45)
							->setCellValue('C'.$y, $valijas->numero_paquetes)
							->setCellValue('D'.$y, date("d/m/Y H:i:s",strtotime($valijas->fecha)));
				$y=$y+1;
			}

			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.($y+1),"Total de registros: ".count($sql_valija_cerrada));
