<?php
	...
	if (empty($sql_paquete)) {
		...
		if (!empty($sql_consolidado)) {
			foreach ($sql_consolidado as $key => $consolidado) {
				$id_consolidado=$consolidado->id_consolidado;
				$id_valija_old=$consolidado->id_valija;
			}

			$db->prepare("UPDATE consolidado SET id_valija=:valija WHERE id_consolidado=:id");
			$db->execute(array(':valija' => $id_valija, ':id' => $id_consolidado));

			$db->prepare("UPDATE paquete SET id_valija=:valija WHERE id_consolidado=:id");
			$db->execute(array(':valija' => $id_valija, ':id' => $id_consolidado));
		...
		if ($id_valija_old!=0) {
			...
			if(!empty($sql_paquete_valija_old)){
				$numero_paquetes_old=count($sql_paquete_valija_old);
