...
// function etiqueta_pdf($id){
    ...
    for ($i=1; $i <($total_paquete+1) ; $i++) {
        ...
        $pdf->Cell(170,20,iconv("UTF-8", "ISO-8859-1", $conf['path_company_name']),0,0,"L");

        if ($id_region==0) {
            $pdf->Cell(26,20,'N/A',1,1,"C");
        }elseif ($id_region==13) {
            $pdf->Cell(26,20,'SCL',1,1,"C");
        }else{
            $pdf->Cell(26,20,'R',1,1,"C");
        }
        ...
        $pdf->MultiCell(196,15,iconv("UTF-8", "ISO-8859-1", $consignatario)." ".$conf['path_cuenta'].$id_cliente,0,"L");
        $pdf->Cell(196,10,'',0,1,"L");

        $pdf->Rect(10, 120, 196, 35, 'D');

        $pdf->SetFont('Arial','',12);
        $pdf->Cell(196,10,"Guia:",0,1,"L");
        $pdf->SetFont('Arial','B',25);
        $pdf->Cell(196,15,iconv("UTF-8", "ISO-8859-1", $tracking_garve),0,1,"C");
        $pdf->Cell(196,10,'',0,1,"L");


        $pdf->Rect(10, 155, 196, 50, 'D');

        $pdf->Cell(196,40,'',0,1,"L");
        $pdf->Image($conf['path_host'].'/miami/etiqueta/barcode.gif',55,157,120);

        $pdf->SetFont('Arial','',15);
        $pdf->Cell(49,10,'Peso(KG):',1,0,"L");
        $pdf->SetFont('Arial','B',20);
        $pdf->Cell(49,10,$peso,1,0,"C");
        $pdf->SetFont('Arial','',15);
        $pdf->Cell(49,10,'Paquete',1,0,"C");
        $pdf->SetFont('Arial','B',20);
        $pdf->Cell(49,10,$i.' de '.$total_paquete,1,1,"C");
        $pdf->Ln();
        $pdf->Ln();
        $pdf->Ln();
        $pdf->Ln();
