...
//PARAMENTROS	
	...
	$db->prepare("SELECT 
		...
		LEFT JOIN gar_usuarios AS cliente ON cliente.id_usuario=paquete.id_usuario
        ...
        INNER JOIN gar_usuarios AS usuario ON usuario.id_usuario=historial.id_usuario

		WHERE historial.id_tipo_status=16
	...
	//CABECERA
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A1', 'Paquetes despachados')
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('J8', 'Peso')
				->setCellValue('K8', 'Usuario');

	//FORMATO DE LOS TITULOS
			$objPHPExcel->getActiveSheet()->mergeCells('A7:K7');
			$objPHPExcel->getActiveSheet()->getStyle('A7:K7')->applyFromArray($styleheaders);
			$objPHPExcel->getActiveSheet()->getStyle('A8:K8')->applyFromArray($styleheaders);
			cellColor("A7:K8", '374E79');

			$y=9;

			foreach ($sql_paquete as $paquete){
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$y, date("d/m/Y H:i:s",strtotime($paquete->fecha)));
				...
				$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('F'.$y, $paquete->nombreCliente.' '.$paquete->apellidosCliente)
					->setCellValue('G'.$y, $paquete->id_usuario)
					->setCellValue('H'.$y, $paquete->descripcion_producto)
					->setCellValue('I'.$y, $paquete->valor)
					->setCellValue('J'.$y, $paquete->peso)
					->setCellValue('K'.$y, $paquete->nombreUsuario.' '.$paquete->apellidosUsuario);
...
header('Content-Disposition: attachment;filename="paquetes_despachados.xls"');
