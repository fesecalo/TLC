<?php
	...
	if(isset($_GET['id_consolidado'])){
		if($_GET['id_consolidado']!=0 || $_GET['id_consolidado']!=''){
			$db->prepare("SELECT * FROM paquete WHERE id_consolidado=:id");
			$db->execute(array(':id' =>$_GET['id_consolidado']));
...
<body>
	...
	<?php if(isset($_GET['id_consolidado'])){ ?>
		<?php if(!empty($resPaquetes)){ ?>
			<table>
				<th colspan="3">Paquetes ingresados en consolidado</th>
				<tr>
					<td>CHI</td>
					<td>TLC tracking</td>
					<td>Peso(KG)</td>
				</tr>
				<?php foreach ($resPaquetes as $key => $paquete) { ?>
					<tr>
