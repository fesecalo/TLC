	...
	<form action="procesa_paquete_counter.php" id="procesa_paquete" name="procesa_paquete" method="POST" >
		<center>
			<table >
				<tr>
					<td>
						<h2>INGRESE O ESCANEE CODIGO ETIQUETA</h2>
		...
		<?php if(!empty($sql_paquete)){ ?>
			<table>
				...
				<tr>
					...
					<td><a href="<?php echo $conf['path_host_url'] ?>/miami/etiqueta/imprime_etiqueta.php?paquete=<?php echo $_GET['id_paquete']; ?>&total=<?php echo $sql_paquete[0]->pieza;?>&barcode=<?php echo $sql_paquete[0]->tracking_garve;?>" target="_blank" class="button solid-color">Imprimir</a></td>
