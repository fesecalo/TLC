<?php
	...
	$id_usu=$sql_paquete[0]->id_usuario;
	...
	if( !isset($_FILES['comprobante0']) ){
		...
		for ($i=0; $i < $cantidad_comprobantes; $i++) {
			...
			if(($tamano <= $limite) AND ($extension=="pdf" || $extension=="PDF" || $extension=="jpg" || $extension=="JPG" || $extension=="xlsx" || $extension=="XLSX" || $extension=="xls" || $extension=="XLS" || $extension=="docx" || $extension=="DOCX" || $extension=="doc" || $extension=="DOC")){ //comprueba si el archivo ingresado es un pdf o jpg de 10mb
				...
				if(!file_exists( $conf['path_files_factura'].'/'.$id_usu."/".$id) ){
					mkdir($conf['path_files_factura'].'/'.$id_usu."/".$id, 0777, true);
				}
				// fin crear carpeta
				...
				if ($extension=="pdf" || $extension=="PDF") {
					...
					while(file_exists( $conf['path_files_factura'].'/'.$id_usu."/".$id."/".$nombre_nuevo)){
				...
				move_uploaded_file($nombre_tmp,$conf['path_files_factura'].'/'.$id_usu."/".$id."/".$nombre_nuevo);
				// fin mover archivos
				...
				// ingreso de registros en la tabla comprobante_compra
				$db->prepare("UPDATE `paquete` SET `valor`=:valor WHERE `id_paquete`= :id_paquete;
				",true);
				$db->execute(array(
					':valor' =>$valorFactura,
					':id_paquete' => $id
				));
				// Actualizar valor del paquete en  tabla paquete
