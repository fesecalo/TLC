...
//PARAMENTROS	
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('F1', 'COMUNA')
				->setCellValue('G1', 'SHIPPER')
				->setCellValue('H1', 'TIENDA')
				->setCellValue('I1', 'DESCRIPCION')
				->setCellValue('J1', 'VALOR USD')
				->setCellValue('K1', 'PESO KG')
				->setCellValue('L1', 'CANTIDAD')
				->setCellValue('M1', 'TRACKING')
				->setCellValue('N1', 'VALIJA');
	...
	//FORMATO DE LOS TITULOS
				...
				foreach ($sql_paquete as $key => $paquete) {
					...
					$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$y, $paquete->id_usuario);
					if(empty($paquete->tracking_garve)){
						$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$y, $paquete->numero_miami);
					}else{
						$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$y, $paquete->tracking_garve);
					}
					$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue('C'.$y, $paquete->nombre." ".$paquete->apellidos)
								->setCellValue('D'.$y, $paquete->rut)
								->setCellValue('E'.$y, $paquete->direccion)
								->setCellValue('F'.$y, $paquete->nombre_comuna)
								->setCellValue('G'.$y, $paquete->nombre_currier)
								->setCellValue('H'.$y, $paquete->proveedor)
								->setCellValue('I'.$y, $paquete->descripcion_producto)
								->setCellValue('J'.$y, $paquete->valor)
								->setCellValue('K'.$y, $paquete->peso)
								->setCellValue('L'.$y, $paquete->pieza)
								->setCellValue('M'.$y, $paquete->tracking_eu)
								->setCellValue('N'.$y, $paquete->cincho);
					$y=$y+1;
				} 
