...
//PARAMENTROS	
	...
	$sql_paquete=$db->get_results("SELECT 
        ...
        INNER JOIN status_log AS historial ON (historial.id_paquete=paquete.id_paquete AND historial.id_tipo_status=2)
	...
	//CABECERA
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A1', 'Paquetes procesados en Miami')
				->setCellValue('A2', 'Fecha de informe: '.date('d/m/Y'))
				->setCellValue('A3', $conf['path_company_name']);

	//TITULOS
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A7', 'Paquetes procesados en Miami')
...
header('Content-Disposition: attachment;filename="Paquetes_procesados_miami.xls"');
