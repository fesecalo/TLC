...
//PARAMENTROS	
	...
	$sql_paquete=$db->get_results("SELECT 
        ...
        INNER JOIN status_log AS historial ON (historial.id_paquete=paquete.id_paquete AND (historial.id_tipo_status=7 OR historial.id_tipo_status=9 OR historial.id_tipo_status=10 OR historial.id_tipo_status=11 OR historial.id_tipo_status=12))
		...
		WHERE (paquete.status=7 OR paquete.status=9 OR paquete.status=10 OR paquete.status=11 OR paquete.status=12)
	...
	//CABECERA
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A1', 'Paquetes retenidos en aduana')
				->setCellValue('A2', 'Fecha de informe: '.date('d/m/Y'))
				->setCellValue('A3', $conf['path_company_name']);

	//TITULOS
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A7', 'Paquetes retenidos en aduana')
	...
	//FORMATO DE LOS TITULOS
			...
			$objPHPExcel->getActiveSheet()->getStyle('A7:I7')->applyFromArray($styleheaders);
			$objPHPExcel->getActiveSheet()->getStyle('A8:I8')->applyFromArray($styleheaders);
			cellColor("A7:I8", '374E79');
			...
			$objPHPExcel->getActiveSheet()->getStyle('A9:I'.($y-1))->applyFromArray($styleText4);
			$objPHPExcel->getActiveSheet()->getStyle('A9:I'.($y-1))->getAlignment()->setWrapText(true);
...
header('Content-Disposition: attachment;filename="Paquetes_retenidos_aduana.xls"');
