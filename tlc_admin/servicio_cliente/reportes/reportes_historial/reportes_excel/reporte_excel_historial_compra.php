...
//PARAMENTROS	
	...
	$db->prepare("SELECT 
		...
		AND paquete.id_usuario=:id_cliente
        GROUP BY paquete.id_paquete
		ORDER BY paquete.fecha_registro ASC
		LIMIT 1000
	");
	$db->execute(array( 
		':inicio' => $fecha_inicio, 
	...
	//CABECERA
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A1', 'Historial de compra cliente')
	...
	//TITULOS
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A7', 'Historial de compra cliente')
	...
	//FORMATO DE LOS TITULOS
			...
			foreach ($sql_paquete as $paquete){
				$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('A'.$y, date("d/m/Y H:i:s",strtotime($paquete->fecha_registro)));
...
header('Content-Disposition: attachment;filename="Historial_compra_cliente.xls"');
