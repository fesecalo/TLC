<?php
	...
	for ($i=1; $i <= $total_paquetes; $i++) {
		if (isset($_POST['entregar'.$i])) {
			...
			$db->prepare("UPDATE paquete SET id_entrega_sin_pago=:id_entrega_sin_pago,envio_entregado=1,status=22 WHERE id_paquete=:id");
			$db->execute(array(':id' => $id_paquete,':id_entrega_sin_pago' => $id_transaccion));
	...
	$db->prepare("SELECT 
		...
		WHERE paquete.id_entrega_sin_pago=:id_transaccion
