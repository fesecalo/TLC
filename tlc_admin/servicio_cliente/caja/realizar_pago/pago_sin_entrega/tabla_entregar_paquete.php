<?php
	...
	$db->prepare("SELECT 
		...
		AND paquete.status!=8
		AND paquete.status!=9
		AND paquete.status!=15
		AND paquete.status!=16
		AND paquete.status!=17
		AND paquete.status!=21
		AND paquete.status!=22
		AND paquete.status!=23)
		AND paquete.cancelado=0
		AND cargo.eliminado=0
		AND paquete.id_transaccion=0
	...
	$(document).ready(function(){
		...
		$("#quitarTodo").click(function(){
	 	...
	 	$("#entregar_paquetes").click(function(){
	 		if (confirm('Se registrara un pago para los paquetes seleccionados y estos quedaran en estado "Pago sin entrega". Desea continuar?')){
