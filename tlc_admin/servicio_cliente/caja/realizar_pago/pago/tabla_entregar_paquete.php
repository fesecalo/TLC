<?php
	...
	$db->prepare("SELECT 
		...
		AND (paquete.status=16 OR paquete.status=22)
	...
	$(document).ready(function(){
		...
		$("#quitarTodo").click(function(){
	 	...
	 	$("#entregar_paquetes").click(function(){
	 		if (confirm('Se realizara un pago para los paquetes seleccionados y pasaran a estado "Entregado y Pagado". Desea continuar?')){
...
<body>
	...
	<form id="entregar" name="entregar" action="procesa_entregar.php" method="post" >
		...
		<table>
			...
			<?php }else{ ?>
				...
				<tr>
					...
					<?php if($paquete->status==22){ ?>
						<td>Entregado pendiente de pago</td>
					<?php }else{ ?>
						<td>Paquete despachado</td>
					<?php } ?>
		...
		<center><input type="button" class="button solid-color" name="entregar_paquetes" id="entregar_paquetes" value="Realizar Pago"></center>
	</form>
	<!-- Fin de contenido -->
