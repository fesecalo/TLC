...
// GLOBAL variables
var scImgPath = '../wimages/spiffyCal_v2_1/';
var hideMethod = 'normal'; //definimos como variable global... lrf
...
function spiffyCalManager() {
	...
	this.matchedFormat="";
	this.DefBtnImgPath=scImgPath; //'./images/';
	...
	this.Holidays = new Array("Dic-25");
	this.HolidaysDesc = new Array("Navidad");
...
'MMM/dd/yyyy'
	...
	function validateDate(eInput, bRequired, dStartDate, dEndDate){
		...
		var rangeMsg = 'Sólo se aceptan fechas entre:\n\n   '+
		...
		if (formatMatchCount>1) {
			...
			if (this.showHelpAlerts) {
				...
				if (date1.getTime()!=date2.getTime()) {
					var Msg='Ha ingresado una fecha ambigua.\n\n Haga click en OK para:\n'+ choice1 +'\n\no Cancel para:\n'+choice2;
					if (confirm(Msg)) {
						bOK=true;
					}
					else {
		...
		if (bOK==true) {
			...
			if (dStartDate!=null) {
				...
				if (dStartDate>dThis){
					...
					if (this.showHelpAlerts) { alert(rangeMsg + 'antes de  '+ strStart + '.');}
					eInput.focus();
					eInput.select();
					return false;
				}
			...
			if (dEndDate!=null) {
				...
				if (dEndDate<dThis) {
					eInput.className = "cal-TextBoxInvalid";
					if (this.showHelpAlerts) {  alert(rangeMsg +'después de  '+ strEnd + '.');}
					eInput.focus();
					eInput.select();
					return false;
				}
			...
			if (bRequired && bIsEmpty) {
				eInput.className = "cal-TextBoxInvalid";
				if (this.showHelpAlerts) {
					alert('Esta fecha es requerida.\n\nPor favor ingrese una fecha válida antes de continuar.');
				}
			}
			else {
				if (!bRequired && bIsEmpty) {
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	var msDOW       = new makeArray0('L','M','W','J','V','S','D');
	...
	function makeCalendar(intWhatMonth,intWhatYear,bViewOnly) {
		...
		for (intLoop=1; intLoop<=msDays[intWhatMonth]; intLoop++, intColumn++) {
			if ((intColumn ==6) || (intColumn ==5)) {isWE=true } else {isWE=false}
...
function dateAdd(intval, numb, base){
	...
	function toggleCombos(showHow){
		//lrf
		//hidemethod se usa solo para hns, si es block viene de hns por lo cual en vez de usar visibility = showhow usa 
		//display = neshowhow... asi oculta todo. ya que por usar layer me marcara error y me desplegara todos los valores
		// ver custom tags select_fecha_stratplanner_hns.cfm
		if(hideMethod=='block'){
			if(showHow=='hidden'){
				newShowHow='none';
			}
				...
				for (j=0;j<document.forms[i].elements.length;j++) {
					if (document.forms[i].elements[j].tagName == "SELECT") {
						if (document.forms[i].name != "spiffyCal") {
							...
							// lo comentado es por JPL
							
								if ( /* ((cboX>=scX-15) && (cboX<=scX+200)) && */ ((cboY>=scY-15) && (cboY<=scY+290)) )
									document.forms[i].elements[j].style.display=newShowHow;
